...
The volume control code modifications as discussed in the issues linked above, was added to the windows port as linked above. The Windows10-based STM32CubeIDE project is here as F411_USB_I2S.zip and the binaries are in build.zip - the hex file can directly be uploaded via the STM32CubeProgrammer and a standard ST-Link. Note that the license applicable is the original source code license. The [**current source**](https://github.com/har-in-air/STM32F411_USB_AUDIO_DAC) linux binaries are included here as build-linux3.tar.gz - this version was built on both Linux Mint with STM32CubeIDE version 1.11.2, and also build-win10 (based on project files STM32F411_USB_AUDIO_DAC-win10.zip), built on windws 10 using the same STM32CubeIDE.
